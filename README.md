...
- **単位が 2 つのエンドポイントで違う**。`getSurveyData` の `wbgt_WO` / `wbgt_Tw` / `wbgt_Tg` は摂氏の小数（`"21.9"`）だが、`getForecastData` の `forecast_val` は**摂氏 ×10 の整数**（`"220"` = 22.0 ℃）。仕様書はどちらの単位も書いていないので、生きた API に照会して確定させた（[PROVENANCE.md](PROVENANCE.md) 問題 4）。仕様書が「数値」と書くフィールドの多くは JSON では文字列で返る
