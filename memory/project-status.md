...
- **次に行う作業（1 つ）**: `TODO.md` #13（`mntr/dl/` の実測地点は 11 か 47 か。配布マスタの `romaji` を先頭大文字に直して `read_moe_wbgt(type = "observe", station = ...)` を数本叩けば決まる）。#10 の PR [#7](https://github.com/uribo/moewbgt/pull/7) は **2026-09-05 にマージ済み**（`0eea68e`、8 ジョブ green）。ローカル `main` は同期済みで作業ツリーはクリーン。他に残るのは #11（`final_wbgt_*` の無名列 `X4`）、#7 と #12（`SHA256SUMS` の同梱と凍結物の定期照合。運用の穴として同じ）、#8 の残り（`default_workflow_permissions` を `read` に戻すか、`renv` ジョブの timeout 実測、roxygen2 の版ずれ）、#9（`.vscode` のローカル NOTE）。
