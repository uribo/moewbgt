...
- **次に行う作業（1 つ）**: `356aebd` を push して 3 ワークフロー（`R-CMD-check` / `renv` / `air-format`）が緑になることを確認する。その後は `TODO.md` #6 の残り（過去年度 CSV / マニュアル PDF の配布継続、`wbgt_point_master-20260515.csv` の列構成、旧 CSV パス体系）か #7（`SHA256SUMS` の同梱と定期照合）のどちらかに進む。
