...
- **次に行う作業（1 つ）**: PR [#4](https://github.com/uribo/moewbgt/pull/4) は 2026-09-04 にマージ済み（`052b45f`、8 ジョブ green）。次は `TODO.md` #6 に残した判断（`date_from == date_to` を受け付けるよう `to <= from` ガードを緩めるか、ユーザー判断）を決めるか、#6 の残り（過去年度 CSV / マニュアル PDF の配布継続、`wbgt_point_master-20260515.csv` の列構成、旧 CSV パス体系）か #7（`SHA256SUMS` の同梱と定期照合）に進む。
