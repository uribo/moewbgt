...
- **次に行う作業（1 つ）**: `TODO.md` #10（直接配布の `wbgt_point_master-20260515.csv` へ乗り換えるか）を決める。**導出元 PDF の保全は 2026-09-05 に完了**（下記）。#6 は 2026-09-05 に**完了**した（上流 3 件を確認、`date_from == date_to` は現状維持で決着）。他に残るのは #11（`final_wbgt_*` の無名列 `X4`）、#7（`SHA256SUMS` の同梱と定期照合）と #12（凍結マニュアルの定期照合。運用の穴として #7 と同じ）、#8 の残り（`default_workflow_permissions` を `read` に戻すか、`renv` ジョブの timeout 実測、roxygen2 の版ずれ）、#9（`.vscode` のローカル NOTE）。**この変更はまだコミットしていない**（`CLAUDE.md` / `PROVENANCE.md` / `TODO.md` / `R/read_moe_{survey,forecast}.R` / `man/` / 新規 `data-raw/verify_frozen_manuals.sh` の 8 ファイル）。
