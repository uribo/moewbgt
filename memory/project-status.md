...
- **次に行う作業（1 つ）**: `TODO.md` #10（直接配布の `wbgt_point_master-20260515.csv` へ乗り換えるか）を決める。**導出元 PDF の保全は 2026-09-05 に完了**（下記）。#6 は 2026-09-05 に**完了**した（上流 3 件を確認、`date_from == date_to` は現状維持で決着）。他に残るのは #11（`final_wbgt_*` の無名列 `X4`）、#7（`SHA256SUMS` の同梱と定期照合）と #12（凍結マニュアルの定期照合。運用の穴として #7 と同じ）、#8 の残り（`default_workflow_permissions` を `read` に戻すか、`renv` ジョブの timeout 実測、roxygen2 の版ずれ）、#9（`.vscode` のローカル NOTE）。この作業は **PR [#6](https://github.com/uribo/moewbgt/pull/6)**（ブランチ `docs/upstream-recheck-2026-09`、4 コミット `dac5240` / `98b4766` / `921840a` / `64b55db`、9 ファイル）として push 済み。**CI は 8 ジョブすべて pass**（R-CMD-check 6 ジョブ 1m26s〜2m23s / renv 1m57s / air-format 7s、2026-09-05）。マージ待ち。
...
- **最後に実行した検証と結果**（2026-09-05）: 上流への読み取り専用の照会を httr2 で実施し、`R03`〜`R09` のマニュアル PDF（403 / 403 / 403 / 403 / 200 / 200 / 404）、`wbgt_point_master-20260515.csv`（200、130,112 バイト、sha256 `684fd9b0…`）、`wbgt_point_master-20250515.csv`（404）を確認。**配布 CSV は scratchpad にだけ置き、`inst/extdata/` には入れていない**（9 個目の凍結バイトを足すかは `TODO.md` #10 の判断）。`R CMD INSTALL --library=<scratchpad>` した版で `read_moe_wbgt()` の 5 分岐と `read_moe_alert()` / `alert_to_long()` の 3 年分（2026 / 2025 / 2021）が通ることを確認。`roxygen2::roxygenise()` → `read_moe_survey.Rd` / `read_moe_forecast.Rd` の 2 件のみ更新（散文の追記だけ）、`air format .` → 差分なし、`testthat::test_dir()` → **PASS 78 / FAIL 0 / WARN 0 / SKIP 0**。なお `R CMD INSTALL` は素の環境だと `~/.Rprofile` の `mcptools::mcp_session()` で落ちる（プロジェクトの `.Rprofile` が読まれないため）。`R_PROFILE_USER=/dev/null R_LIBS=<renv lib>` を付けると通る。
