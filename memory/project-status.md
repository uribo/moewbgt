...
- **次に行う作業（1 つ）**: PR [#8](https://github.com/uribo/moewbgt/pull/8)（`chore/pin-cran-in-renv-update`、`cef8ce2`）をマージする。**CI は 8 ジョブすべて green**。その後は `TODO.md` #13（`mntr/dl/` の実測地点は 11 か 47 か。配布マスタの `romaji` を先頭大文字に直して `read_moe_wbgt(type = "observe", station = ...)` を数本叩けば決まる）。#10 の PR [#7](https://github.com/uribo/moewbgt/pull/7) は **2026-09-05 にマージ済み**（`0eea68e`、8 ジョブ green）。ローカル `main` は同期済みで作業ツリーはクリーン。他に残るのは #11（`final_wbgt_*` の無名列 `X4`）、#7 と #12（`SHA256SUMS` の同梱と凍結物の定期照合。運用の穴として同じ）、#8 の残り（`default_workflow_permissions` を `read` に戻すか、`renv` ジョブの timeout 実測、roxygen2 の版ずれ）、#9（`.vscode` のローカル NOTE）。

- **週次 lockfile 更新で CRAN を固定した（2026-09-05、ユーザー判断。PR #8）**: マージ済みの PR #5 が `diffobj` 0.3.6 → 0.3.8 のほかに 2 つの変更を持ち込んでいた — lockfile の CRAN リポジトリ URL が動く `packagemanager.posit.co/cran/latest` に、11 パッケージの `Repository` が `"CRAN"` → `"RSPM"` に。原因は `renv-update.yaml` の update ステップが `Sys.getenv("RSPM", ...)` でランナーの PPM を向いていたこと。`options(repos = c(CRAN = "https://cloud.r-project.org"))` に固定し、その固定のもとで `renv::snapshot()` を回し直して lockfile を正規化した（**手編集していない**。版・パッケージ集合は不変、差分はリポジトリ URL 1 行と 12 レコードの `Repository` だけ）。update ジョブの `timeout-minutes` は 45 → 90。**ローカル環境も同期済み**（`renv::restore()` で `diffobj` 0.3.8、`renv::status()` は No issues found）。なお `renv` ジョブは CRAN 固定の lockfile でも 2m7s で通っており、renv 側の PPM 変換が Linux バイナリを供給していると見られる（pak 経由の update ジョブで同じかは未確認。90 分はその保険）。
