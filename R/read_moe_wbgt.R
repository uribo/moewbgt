read_moe_wbgt <- function(
  ...
  rlang::arg_match(type, c("forecast", "observe"))
  if (!is.null(station)) {
    rlang::arg_match(
      station,
      c(
  ...
  if (type == "forecast") {
    ...
    if (year == lubridate::year(lubridate::today())) {
      if (!is.null(station_no) & !is.null(year_month)) {
        ...
        is.null(station_no) & is.null(prefecture) & !is.null(year_month)
      ) {
      ...
      if (!is.null(station_no) & !is.null(year_month)) {
        glue::glue(
          "{domain_url}/mntr/final/{year}/wbgt_{year}/final_wbgt_{station_no}_{year_month}.csv"
        )
      }
    }
  }
