...
pdf_table_to_df <- function(text) {
  ...
  x <-
    x %>%
    stringr::str_subset(
      "^(北海道|東北|関東|甲信|東海|北陸|近畿|中国|四国|九州|沖縄)"
    ) %>%
    purrr::map(
  ...
  d <-
    ...
    stringr::str_subset(
      paste0(
        "(",
        paste0(
          harmonize_prefecture_name(jpnprefs$prefecture_kanji, to = "short"),
          collapse = "|"
        ),
        ")"
      )
...
if (!file.exists(here::here("inst/extdata/wbgt_stations2024.csv"))) {
  ...
  d <-
    collect_wbgt_stations(
      here::here("data-raw/R06_wbgt_data_service_manual.pdf"),
      17,
      29,
      ignores = c("34361", "41171", "48541", "84356", "84596", "86156")
    ) |>
...
if (!file.exists(here::here("inst/extdata/wbgt_stations2023.csv"))) {
  ...
  d <-
    collect_wbgt_stations(
      here::here("data-raw/R05_wbgt_data_service_manual.pdf"),
      10,
      22,
      ignores = c("34361", "41171", "48541", "84356", "84596", "86156")
    ) |>
...
if (!file.exists(here::here("inst/extdata/wbgt_stations2022.csv"))) {
  ...
  df_wbgt_stations <-
    ...
    inner_join(
      ...
      by = c("station_no" = "地点番号", "station_name" = "観測所名")
    ) |>
