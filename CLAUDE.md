...
- `tests/` が無い。testthat も `DESCRIPTION` に入っていない
...
## SHA256SUMS の扱い（重要・kumagusu と規約が違う）

`SHA256SUMS` の 13 行は**性質の異なる 2 種類が混在している**。同じ「ダイジェスト記録」に見えるが、更新してよいかが逆になる。

- **`inst/extdata/` の 8 行 = 凍結バイト**。環境省の過去年度版マスタ CSV とマニュアル PDF が今も配布されているかは未確認で、配布されていなければ再取得不能。**不一致を記録値の書き換えで解消しない。ファイルを削除・上書きしない。** 不一致は「上流または手元が変わった」という事実の報告であり、何が変わったかを確認してユーザーに報告する
- **`R/*.R` と `data-raw/*.R` の 5 行 = コピー忠実性の記録**。japan-heatstroke からバイト一致でコピーしたことの証拠でしかない。`wbgt_guideline()` の境界（PROVENANCE 問題 7）や memoise のトップレベル呼び出し（同 2）を直せば `shasum -c` はこの 5 行で**正しく失敗する**。これは異常ではない

この混在自体が設計の穴なので、コード側を最初に修正するときに決着させる。**勝手に決めずユーザーに委ねる**（`TODO.md` に起票済み）。
...
- `data-raw/moe_wbgt_stations.R` — 提供サービスマニュアル PDF から地点マスタを抽出する導出スクリプト。**出力先が `here::here("data/…")` のままで `inst/extdata/` と揃っていない**（PROVENANCE 問題 1）
...
- **`R/` の日本語文字列リテラルは Unicode エスケープで書く**（`"危険"` ではなく `"\u5371\u967a"` の形）。R CMD check の非 ASCII 警告を避けるため。現状 `R/guides.R:11-14` の 4 語と `R/moe_alert.R:27` の `"発表回数"` が生のまま残っている（同ファイルの `"\u25cf"` は既にエスケープ済み）。触るときに直す。`data-raw/` は `.Rbuildignore` されるので生のままでよい
