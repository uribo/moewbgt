...
**扱い: 完了（2026-09-05、ユーザー判断）** — 乗り換えた。生バイトを凍結し、そこから使えるデータセットを導出する形（選択肢 A）。

- `inst/extdata/wbgt_point_master-20260515.csv` を **9 個目の凍結バイト**として追加し、`SHA256SUMS` に 1 行足した（`shasum -c` は 9/9 OK）。上流は掲載日入りの 1 本しか無く前年度版は既に 404 なので、凍結する理由の方が強い
- 導出は `data-raw/wbgt_stations.R`（`file.exists()` ガード＋ sha256 不一致で `stop()`）。日本語ヘッダーの並びを照合してから位置で英語名に読み替え、緯度経度を 10 進度へ畳み、番人値 `9999-99-99` を `NA` にし、`wbgt_pref_codes` から `area_cd` / `pref_cd` を結合する
- 出力は `data/wbgt_stations.rda`（865 行 × 18 列）。`station_no` は WebAPI の `wbgt_no` と同じ整数なので返り値に直接結合できる。ドキュメントは `R/data.R`、テストは `tests/testthat/test-data.R`（3 ブロック追加、全体で PASS 97）
- **既存の 8 ファイルは削除も改名もしない**（再取得不能な年度スナップショット。公開リポジトリの破壊的変更でもある）。`data-raw/moe_wbgt_stations.R` も残す
- 副産物: 配布 CSV は `実測開始日`（49 地点・稼働中 47）、`特別警戒情報判定除外`（24 地点）、`Old Station Number` の振り直し（4 件）を持つ。前者は #13 へ
...
**扱い: 次に回す**（2026-09-05 の #10 で気づいた。現行コードは壊れておらず、地点名を明示すれば動く）
